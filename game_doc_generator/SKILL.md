...
**1. 读取全局基础机制与经验库** ⚠️ **必须执行**
   - 必须强制读取项目根目录下的 `复盘记录/AI经验学习库.md`，在后续生成中严格避开记录中的错误惯例，遵循已沉淀的常态化规则。
   - 必须强制读取项目根目录下的 `框架/游戏结构总览.md`，建立对游戏整体模块分布和资源流向的宏观认知。

**2. 检索并读取特定功能的会议记录** ⚠️ **必须执行**
   - 检索 `会议记录/` 目录下是否有与当前工作目标相关的历史讨论或会议记录文件。
   - **核心参考约定**：要求把检索到的《会议记录》作为生成和修改文档的重要补充参考依据，但如果出现冲突，**不要强行使用会议记录约束或覆盖原定文档框架，仅作为设计参考**。
...
根据步骤1确定的文档模式选择对应的结构：
- **框架设计文档**：使用【模式一：框架设计文档结构】。通俗易懂，自然段落呈现，禁止生硬堆砌“一、二、三”子标题。
- **开发文档**：使用【模式二：开发文档结构】。保留所有的编号章节体系，确保细节详尽落地。
...
#### 步骤7：同步维护《游戏结构总览》 ⚠️ **最后落幕必须执行**

本次文档生成或基于用户反馈的迭代修改（如有）全部定稿完成后：
1. 必须读取并分析 `框架/游戏结构总览.md`。
2. 将本次新增的功能模块挂载到正确的系统架构树下，或更新现有模块的关联流向和重点资源（如经济产出/消耗）流经点。
3. 动态更新该总览文件，并记录落款时间。维持其作为整个开发空间全局最新宏观蓝图的状态。
...
5. **通俗化用词与反过度包装** ⚠️ **严格要求**：
   - 严禁使用生僻、抽象的“系统大词”或“自造词”（如把“容量上限”说成“容量规划与经济出口”，把“分类”说成“筛选体系”，把“命名规则”说成“名称拼装体系”，把“散件”说成“孤件装备”）。
   - 严禁在描述程序实现逻辑时过度包装或生造术语（如把“优先级判断”说成“顺序解套排摸阵位”，把“容错机制”说成“安全防侧翻兜底”）。
   - 必须使用最通用、直白、口语化且易懂的游戏与技术术语。
   - 绝不使用主观情绪化的描述（如“此容量并不宽裕”，应客观描述为“初始容量较小”）。
6. **纯文本描述** ⚠️ **严格要求**：
   - **绝对禁止使用代码或伪代码**来描述规则
   - 必须使用自然语言、表格、列表来表述规则
   - 数值计算使用数学公式格式
...
✅ **必须包含的章节**：文档信息与更新记录（开发文档强制）、设计目的、功能概述、规则说明、策划需求
