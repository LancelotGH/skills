...
本 skill 用于指导 AI 助手**从用户提供的游戏功能需求文档生成专业的游戏功能设计文档**。
...
- `game_doc_template` — 文档结构模板（可独立调用查阅模板）
- `game_rule_writing` — 规则编写规范（可独立调用评审规则）
- `game_config_table` — 配置表设计指南（可独立调用设计配置表）
- `game_doc_quality` — 质量检查与关联校验（可独立调用做质量审查）
