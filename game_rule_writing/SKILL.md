...
- ❌ **造词/AI 倾向**：”端实值演化”、”行为因子”、”不强制耦合”、”满溢”、”落账”、”升格校验”、”终局结算”、”耗竭降级”
- ✅ **正确/通俗替换**：”性格实值更新”、”玩家行为数据”、”无强制关联”、”XP 达到满值”、”计入”、”晋升结算”、”Token 不足时判定”

**名词定义常见拗口改写示例（AI 生成需重点核查）**：

| 错误写法 | 正确写法 | 说明 |
| :--- | :--- | :--- |
| 人格实值 | 性格实值 | “人格”偏心理学，”性格”更口语直白 |
| 各属性实际生效系数 | 综合偏向系数 | 既精简又与公式一致，消除双名 |
| 人格升格 | 阶段升级 | “升格”生僻，”升级”通俗 |
| 升格任务 / 人格升格任务 | 阶段晋升任务 | 同上 |

### 3. 禁用研发向的抽象造词与过度包装
在描述程序与系统交互逻辑时，禁止生造晦涩难懂的“大词”，必须还原为直白的动作或状态。
- ❌ **错误/抽象造词**：“全盘唤醒扫描起点节点”、“顺序解套排摸阵位等级”
- ✅ **正确/平实描述**：“触发起点”、“合成优先级判定”

- ❌ **错误/后端行话**：“通过相同的判断总线来处理目标的下发、核销”、“兜底防爆透传”
- ✅ **正确/直白逻辑**：“使用同一套机制处理派发和进度累加”、“前端异常强保护层”

- ❌ **错误/抽象造词**：“切换流与持久判定”、“系统核算积攒的临时缓存池包包加法账户”
- ✅ **正确/平实描述**：“开关设定与持久化”、“系统计算暂存的属性点数值”

### 6. 触发逻辑就地归属原则

当某个模块会产生外部触发（如触发 LLM 同步、触发结算、发送通知）时，**该触发描述必须写在触发来源的模块内**，不能单独拉出"触发机制"章节汇总。

- ❌ **错误做法**：新建"大模型同步触发机制"章节，将所有触发节点集中列出
- ✅ **正确做法**：
  - 期望偏向级升级触发同步 → 写在"等级与偏向系数"小节内
  - 装备变更触发同步 → 写在"装备期望加成"小节内
  - 晋升结算触发同步 → 写在"晋升结算"小节内

**理由**：读者在看某个模块时就能知道"这个操作会触发什么"，不需要跳转到其他章节拼图。

### 7. 术语首次出现就地定义原则

文档中出现合成词或非通用术语时，**必须在该术语第一次出现处紧跟括号说明组成内容**，不得推迟到后续段落另行解释。

- ❌ **错误做法**：2.3 节写"参与综合偏向系数的计算"，直到 4.2 节才给出 `综合偏向系数 = A + B + C`
- ✅ **正确做法**：2.3 节写"参与**综合偏向系数**（期望偏向级系数 + 装备偏向加成 + 心情修正值）的计算"

**补充**：括号定义只需简短列出组成项，不用额外写"其中……"解释句，避免啰嗦。

### 4. 彻底的"脱水精炼"（大白话+结果导向）原则
文档虽然要通顺，但绝不能把文档写成“聊天式的流水账”。去除所有多余的副词、主观语气和拖沓的长句。
- **准则**：不教程序如何写底层架构，不带冗余口水话，只定死明确的**判定条件**和**业务结果**。
- ❌ **错误/啰嗦口语+技术词汇**：“等网络好了或者玩家重连了，客户端悄悄读取这份云段滞压缓发记录，正常重新把动作完整表演出去保底。”
- ✅ **正确/极简精干事实**：“断线重连后，客户端读取并自动补播未弹出的对话与动作。”
...
### 5. 禁用带有负面情绪或现实借贷色彩的商业用语
在描述游戏内资源暂扣、离线数据延迟同步等机制时，严禁使用带有现实经济负债压力的词汇，必须使用中性的技术规范用语。
- ❌ **错误/负债词汇**：“欠款”、“记账”、“欠账补偿”、“打欠条”、“平账”
- ✅ **正确/技术规范**：“数据暂存”、“后台缓存”、“待同步队列”、“静默合并扣除”、“挂起”
