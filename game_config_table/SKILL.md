---
name: 游戏配置表设计指南
description: 提供游戏配置表结构的设计规范，包含复用原则、字段设计、命名规范和常见类型参考。当用户要求设计配置表、修改配置表、审查配置表，或说”配置表字段不合理”时，应使用此 skill。
---
...
本 skill 提供**配置表结构的设计规范和最佳实践**，确保配置表设计规范、可维护、易扩展。需要设计或审查配置表时手动调用。
